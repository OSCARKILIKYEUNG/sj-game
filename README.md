...
- `index.html` is a **build artifact** (v4, 2026-07-16). The source of truth (ES-module `src/`, esbuild step, 62-check headless test suite) lives in a private repo; to update, build there (`node build.mjs`) and copy the output over this file.
...
Debug URL params of the current build: `?start=1` 繚 `?boss=1|2` 繚 `?h=N` (start N points up) 繚 `?gallery=1` 繚 `?freeze=N` 繚 `?touch=1`.
