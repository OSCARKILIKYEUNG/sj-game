...
- `index.html` is a **build artifact** (v3, 2026-07-13). The source of truth (ES-module `src/`, esbuild step, 62-check headless test suite) lives in a private repo; to update, build there (`node build.mjs`) and copy the output over this file.
...
Debug URL params of the current build: `?start=1` · `?boss=1|2` · `?h=N` (start N points up) · `?gallery=1` · `?freeze=N` · `?touch=1`.
